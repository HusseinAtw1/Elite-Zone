            ...
            success: function(response) {
                ...
                response.forEach(function(message) {
                    if (message.sent_message) {
                        chatContent.append(
                            '<div class="chat-message user-message text-left mb-2">' +
                    ...
                    if (message.reply_message) {
                        chatContent.append(
                            '<div class="chat-message admin-message text-right mb-2">' +
    ...
    $('#messageform').on('submit', function(e) {
        ...
        if (message) {
            $.ajax({
                url: '../handle.php',
                method: 'POST',
                data: { chatId: chatId, messageadmin: message },
                success: function(response) {
                    let newMessage = '<div class="chat-message admin-message text-right mb-2">' +
                                        '<div class="message-content d-inline-block bg-primary text-white p-2 rounded">' +
                                            message +
                                        '</div>' +
                                        '<div class="message-time small text-muted mt-1">' +
